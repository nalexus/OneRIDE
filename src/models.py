...
class MinigridPolicyNet(nn.Module):
    def __init__(self, observation_shape, num_actions):
        ...
        self.fc = nn.Sequential(
            init_(nn.Linear(32, 512)),
            nn.ReLU(),
            init_(nn.Linear(512, 512)),
            nn.ReLU(),
        )

        self.core = nn.LSTM(512, 512, 1)
        ...
        #self.intent_random = init_(nn.Linear(1024, 2))
    ...
    def forward(self, inputs, core_state=()):
        ...
        for input, nd in zip(core_input.unbind(), notdone.unbind()):
            ...
            output, core_state = self.core(input.view(1,B,512), core_state)
            core_output_list.append(output)
        core_output = torch.flatten(torch.cat(core_output_list), 0, 1)
        
        policy_logits = self.policy(core_output)
        baseline = self.baseline(core_output)
        #intent_random = self.intent_random(core_output)

        if self.training:
            # decision_random = torch.multinomial(
            #     F.sigmoid(intent_random), num_samples=1)
            #if decision_random
            action = torch.multinomial(
                F.softmax(policy_logits, dim=1), num_samples=1)
            # print('ACTION', action)
            # print('policy_logits', policy_logits)
            # if decision_random.numpy()[0][0]==1:
            #     action = torch.multinomial(
            #         torch.tensor([[1/self.num_actions for i in range(self.num_actions)] for i in range(policy_logits.shape[0])]), num_samples=1)
            #     # print('RANDOM', decision_random)
            #     print('ACTION_new', action)
        else:
            # decision_random = torch.multinomial(
            #     F.sigmoid(intent_random), num_samples=1).numpy()[0][0]
            # if decision_random==1:
            #     action = torch.multinomial(
            #         torch.tensor([[1 / self.num_actions for i in range(self.num_actions)]]), num_samples=1)
            # else:
            action = torch.argmax(policy_logits, dim=1)
...
class MinigridAdverseStateClassifier(nn.Module):
    def __init__(self, observation_shape):
        super(MinigridAdverseStateClassifier, self).__init__()
        self.observation_shape = observation_shape
        ...
        self.binary_classifier = init_(nn.Linear(128, 2))

    def initial_state(self, batch_size):
        return tuple(torch.zeros(self.core.num_layers, batch_size,
                                 self.core.hidden_size) for _ in range(1))
    ...
    def forward(self, inputs):
        ...
        core_output_list = []
        #notdone = (~inputs['done']).float()
        for input in zip(core_input.unbind()):
            # nd = nd.view(1, -1, 1)
            # self.core_state = tuple(nd * s for s in self.core_state)
            if not self.core_state:
                output, self.core_state = self.core(input[0].view(1,B,128))
            else:
                output, self.core_state = self.core(input[0].view(1,B,128),self.core_state)
            core_output_list.append(output)
        core_output = torch.flatten(torch.cat(core_output_list), 0, 1)
        adversarial_logits = self.binary_classifier(core_output)
        # adversarial_logits.requires_grad = False
        state_class_prob = F.sigmoid(adversarial_logits)
        preds = torch.multinomial(
            state_class_prob, num_samples=1)
        # preds.requires_grad = True
        preds = preds.view(T, B)
        adversarial_logits = adversarial_logits.view(T, B, 2)

        return adversarial_logits,preds
...
class MinigridGoalNet(nn.Module):
    def __init__(self):
        super(MinigridGoalNet, self).__init__()
    ...
    def forward(self, state_embedding):
        next_goal_emb = self.fd_out(self.forward_dynamics(state_embedding))
        return next_goal_emb


class MinigridBackwardDynamicsNet(nn.Module):
    def __init__(self, num_actions):
        super(MinigridBackwardDynamicsNet, self).__init__()
        self.num_actions = num_actions
