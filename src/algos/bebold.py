...
import src.models_bebold as models
import src.losses_bebold as losses

from src.env_utils import FrameStack
from src.utils_bebold import get_batch, log, create_env, create_buffers, act, create_heatmap_buffers
...
def learn(actor_model,
    ...
    with lock:
        ...
        encoded_states, unused_state = encoder(batch['partial_obs'].to(device=flags.device), initial_encoder_state,
                                               batch['done'])
        ...
        intrinsic_rewards_next = torch.norm(predicted_embedding_next.detach() - random_embedding_next.detach(), dim=2,
                                            p=2)
    ...
    def batch_and_learn(i, lock=threading.Lock()):
        ...
        while frames < flags.total_frames:
            ...
            batch, agent_state, encoder_state = get_batch(free_queue, full_queue, buffers,
                                                          initial_agent_state_buffers, initial_encoder_state_buffers,
                                                          flags, timings)
